---
description: Run the full LinkedIn job scraping and evaluation pipeline
---
...
1. **Run the full pipeline** (scrape + evaluate in one command):
   ...
   python main.py --platform linkedin --title "Data Scientist" --location "Bangalore" --max-jobs 5 --no-headless --resume resume.txt
   ```

   This will automatically:
   - **Phase 1**: Open Chrome, navigate to LinkedIn public job search, scroll to collect job cards, open each in a new tab for full description, save all to `data/job_applications.xlsx`, then close the browser.
...
- **No jobs found**: Use `--no-headless` to see the browser. LinkedIn may show a CAPTCHA.
